...
func ListPorts(ctx context.Context, svcCtx *svc.ServiceContext) (*appTypes.PortsResponse, error) {
	...
	result := &appTypes.PortsResponse{Ports: make([]appTypes.PortUsage, 0), Conflicts: make([]string, 0)}
...
func containerPorts(inspect dockerTypes.ContainerJSON) []appTypes.ComposePort {
	...
	for target, bindings := range inspect.NetworkSettings.Ports {
		...
		if len(bindings) == 0 {
			...
			if !seen[key] {
				ports = append(ports, appTypes.ComposePort{ContainerPort: containerPort, Protocol: protocol})
				seen[key] = true
			}
			continue
		}
		for _, binding := range bindings {
			key := binding.HostIP + ":" + binding.HostPort + ":" + containerPort + ":" + protocol
			if seen[key] {
				continue
			}
			ports = append(ports, appTypes.ComposePort{HostIP: binding.HostIP, HostPort: binding.HostPort, ContainerPort: containerPort, Protocol: protocol, Published: binding.HostPort != ""})
			seen[key] = true
		}
	}

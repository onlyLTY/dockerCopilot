...
- 容器 CPU/内存由后端按需采样：有人读取容器列表时每 15 秒采集一轮；持续 2 分钟无人访问自动暂停采样（对 Docker 零调用）。重新访问时内存占用立即可用，CPU 使用率需要两个采样点，约 15–30 秒内自动补齐。
- 前端静态资源按内容哈希长期缓存（immutable），`index.html` 每次回源验证，发布新版本立即生效。

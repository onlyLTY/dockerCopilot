package compose

import (
	"net/http"
	"time"

	"github.com/onlyLTY/dockerCopilot/internal/logic/compose"
	"github.com/onlyLTY/dockerCopilot/internal/svc"
	"github.com/onlyLTY/dockerCopilot/internal/types"
	"github.com/zeromicro/go-zero/rest/httpx"
)

func ComposeDeployPreviewHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		started := time.Now()
		var req types.ComposeDeployPreviewReq
		if err := httpx.Parse(r, &req); err != nil {
			auditError("deploy_preview", r, err, started)
			httpx.ErrorCtx(r.Context(), w, err)
			return
		}
		resp, err := compose.NewComposeDeployPreviewLogic(r.Context(), svcCtx).ComposeDeployPreview(&req)
		audit("deploy_preview", r, resp, started)
		writeLogicResp(w, r, resp, err)
	}
}

...
export class IconService {
  ...
  remove(imageName: string): Observable<ApiResponse<unknown>> { return this.done(this.http.delete<ApiResponse<unknown>>('/api/icons?imageName=' + encodeURIComponent(imageName))); }

  /** 图标增删后：刷新图标自身，并让容器/镜像/端口缓存失效（这些页面卡片图标依赖该 map） */
  private done<T>(obs: Observable<ApiResponse<T>>): Observable<ApiResponse<T>> {
    return obs.pipe(tap(r => { if (r.code === 200) { this.store.refresh(); this.bus.invalidate(['containers', 'images', 'ports']); } }));
  }
  ...
  imageIcon(imageName: string): string {
    const file = builtInImageIcons[this.normalizeRepository(imageName)] || builtInImageIcons['default'];
  ...
  resolve(imageName: string, custom: IconMap): string {
    ...
    const customKey = Object.keys(custom).find(name => this.normalizeRepository(name) === repository);
  ...
  normalizeRepository(imageName: string): string {
    ...
    if (parts.length > 1 && (parts[0].includes('.') || parts[0].includes(':') || parts[0] === 'localhost')) value = parts.slice(1).join('/');

...
export class ComposeComponent {
  ...
  readonly preview = signal<Record<string, any> | undefined>(undefined);
  ...
  openEditor(p: ComposeProject) {
    ...
    const f = normalized.files.find(x => x.name === 'compose.yaml') || normalized.files[0];
  ...
  validate() {
    ...
    runAction({
      ...
      onSuccess: r => {
        const n = Array.isArray((r.data as { services?: unknown[] } | undefined)?.services)
  ...
  validateCreate() {
    ...
    runAction({
      ...
      onSuccess: r => {
        const count = Array.isArray((r.data as { services?: unknown[] } | undefined)?.services)
          ? (r.data as { services: unknown[] }).services.length
          : 0;
  ...
  createProject(deployAfterCreate = false) {
    ...
    runAction({
      ...
      onBizError: r =>
        this.setCreateError(this.formatComposeError(r.msg || '创建项目失败'), 'error'),
      onHttpError: e =>
        this.setCreateError(
  ...
  private runBulkCleanup(projects: ComposeProject[]) {
    ...
    projects.forEach(project =>
      this.service.cleanupPreview(project.id).subscribe({
        next: preview => {
          if (preview.code !== 200 || !preview.data?.['previewToken']) {
            failed++;
            if (!--remaining) this.finishCleanup(projects.length, failed);
            return;
          }
          this.service.cleanup(project.id, String(preview.data['previewToken'])).subscribe({
            next: r => {
              if (r.code !== 200) failed++;
              if (!--remaining) this.finishCleanup(projects.length, failed);
            },
            error: () => {
              failed++;
  ...
  private finishCleanup(total: number, failed: number) {
    this.cleanupBusy.set(false);
    this.exitSelection();
    this.service.refresh();
    if (failed) this.toast.error(`项目清理完成 ${total - failed} 个，失败 ${failed} 个`);
    else this.toast.success(`已清理 ${total} 个项目`);
  }
  closeEditor(e?: Event) {
  ...
  statusLabel(s: string) {
    return (
      (
        { using: '使用中', stopped: '已停止', unused: '未使用', unknown: '未知' } as Record<
          string,
          string
        >
      )[s] || s

...
export class IconsComponent {
  ...
  readonly filtered = computed<[string, string][]>(() => {
    const q = this.query.trim().toLowerCase();
    return Object.entries(this.icons()).filter(item => !q || item[0].toLowerCase().includes(q));
  });
  ...
  readonly allSelected = computed(
    () => this.filtered().length > 0 && this.filtered().every(item => this.selected().has(item[0])),
  );
  toggleAll() {
    const next = new Set(this.selected());
    if (this.allSelected()) this.filtered().forEach(item => next.delete(item[0]));
    else this.filtered().forEach(item => next.add(item[0]));
    this.selected.set(next);
  }
  bulkRemove() {
    ...
    this.confirm
      .open({
        title: '批量删除图标',
        message: `确定删除选中的 ${names.length} 个图标吗？`,
        confirmText: '批量删除',
        danger: true,
      })
      .then(ok => {
        if (ok) this.runBulkRemove(names);
      });
  }
  ...
  upload(): void {
    ...
    this.service.upload(this.name.trim(), file).subscribe({
      next: result => {
        this.uploading.set(false);
        if (result.code === 200) {
          this.show.set(false);
          this.toast.success('图标已保存');
        } else this.error.set(result.msg);
      },
      error: error => {
        this.uploading.set(false);
        this.error.set(error.error?.msg || '上传失败');
      },
    });
  }
  async remove(name: string): Promise<void> {
    ...
    this.service.remove(name).subscribe({
      next: result => {
        if (result.code === 200) this.toast.success('图标已删除');
        else this.toast.error(`删除失败：${result.msg || '未知错误'}`);
      },
      error: e => this.toast.error(`删除失败：${e.error?.msg || e.message || '请求错误'}`),
    });
  }
  close(event: Event): void {
    if (event.target === event.currentTarget) this.show.set(false);
  }
  fallback(event: Event): void {
    (event.target as HTMLImageElement).src = this.service.defaultIcon();
  }
}
